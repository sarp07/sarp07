...
import random
W=320
main=[('FAINERA','CTO, CONTRACT','ACTIVE','#7cf2a0'),
      ('ELYON NETWORK','TECH + COMMUNITY GROWTH ADVISOR','ACTIVE','#7cf2a0'),
      ('ZETARIUM','CTO, CONTRACT. PROJECT DISBANDED','ENDED','#8a7aa8')]
done=[];_x=[('AGRO WALLET','WALLET + EXTENSION'),('NFT MARKET V3','NFT MARKETPLACE'),('METACYBER','NFT CARD BATTLE GAME'),('FARMER GAME','P2E NFT STAKING')]
y0=14
mh=len(main)*22+10
dh=0
H=y0+mh+2
...
b.append(window(6,y0-4,308,mh,'GUILDS'))
bang=["GGGGGG","GYYYYG","GYGGYG","GYYYYG",".GYYG.","..GG.."]
for i,(n,d,st,c) in enumerate(main):
    y=y0+6+i*22
    b.append(f'<g class="bang" style="animation-delay:{i*.2}s">{sprite(bang,19,y,{"G":C["gold"],"Y":C["org"]})}</g>')
...
'''
open('assets/guilds.svg','w').write(svg(W,H,''.join(b),style,'Guilds: CTO at FainEra, tech and growth advisor at Elyon Network, former CTO at Zetarium'))
